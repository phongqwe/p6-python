...
class DataCellTest(unittest.TestCase):
    ...
    def test_numeric_str_value(self):
        c1 = DataCell(address = CellIndex(1, 1), wsName = self.wsName,wbKey = self.wbk,value="123",)
        self.assertEqual("123",c1.strValue)
        self.assertEqual("123",c1.displayValue)

        c2 = DataCell(address = CellIndex(1, 1), wsName = self.wsName,wbKey = self.wbk,value="\'abc")
        self.assertEqual("\'abc", c2.strValue)
        self.assertEqual("\'abc", c2.displayValue)

    def test_sourceValue(self):
        c1 = DataCell(address = CellIndex(1, 1), wsName = self.wsName,wbKey = self.wbk,value=123,formula = "formula")
    ...
    def test_content(self):
        c1 = DataCell(address = CellIndex(1, 1), wsName = self.wsName,wbKey = self.wbk,value=None)
        ct1 = c1.content
    ...
    def test_content_setter(self):
        ...
        c1 = DataCell(address = CellIndex(1, 1), wsName = self.wsName,wbKey = self.wbk,value="v")
        c1.content = ct1
    ...
    def test_toProtoObj2(self):
        c1 = DataCell(address = CellIndex(1, 1),
                      wsName = self.wsName,
                      wbKey = self.wbk,
                      value=None)
        proto = c1.toProtoObj()
    ...
    def test_isValueEqual(self):
        c1 = DataCell(address = CellIndex(1, 1), wsName = self.wsName,wbKey = self.wbk,value=123)
        c2 = DataCell(address = CellIndex(4,2), wsName = self.wsName,wbKey = self.wbk,value=123)

        self.assertTrue(c1.isValueEqual(c2))
        self.assertTrue(c2.isValueEqual(c1))
        self.assertTrue(c2.isValueEqual(c2))
        c3 = DataCell(address = CellIndex(2,3), wsName = self.wsName,wbKey = self.wbk,value=-234)
        self.assertFalse(c2.isValueEqual(c3))
        self.assertFalse(c3.isValueEqual(c2))

    def test_setFormula2(self):
        f = "=SUM(A1:B3)"
        c1 = DataCell(address = CellIndex(1,1), wsName = self.wsName,wbKey = self.wbk,formula = f)
        c1.value=123
        c1.formula = f
